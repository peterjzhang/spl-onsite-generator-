...
if 'last_uploaded_file_id' not in st.session_state: # Initialize tracker for uploaded file
...
def populate_session_state_from_config_dict(config_data, s_state, prefix=""):
    ...
    s_state[prefix + "num_trainers"] = sim_settings.get("num_trainers", s_state.get(prefix + "num_trainers"))
    s_state[prefix + "num_reviewers"] = sim_settings.get("num_reviewers", s_state.get(prefix + "num_reviewers"))
    s_state[prefix + "simulation_days"] = sim_settings.get("simulation_days", s_state.get(prefix + "simulation_days"))
    
    tc = config_data.get("trainer_config", {})
    s_state[prefix + "trainer_max_hours_per_week"] = tc.get("max_hours_per_week", s_state.get(prefix + "trainer_max_hours_per_week"))
    s_state[prefix + "trainer_target_hours_per_day"] = tc.get("target_hours_per_day", s_state.get(prefix + "trainer_target_hours_per_day"))
    s_state[prefix + "trainer_target_hours_per_day_noise"] = tc.get("target_hours_per_day_noise", s_state.get(prefix + "trainer_target_hours_per_day_noise"))
    s_state[prefix + "trainer_writing_hours"] = tc.get("writing_hours", s_state.get(prefix + "trainer_writing_hours"))
    s_state[prefix + "trainer_writing_hours_noise"] = tc.get("writing_hours_noise", s_state.get(prefix + "trainer_writing_hours_noise"))
    s_state[prefix + "trainer_revision_hours"] = tc.get("revision_hours", s_state.get(prefix + "trainer_revision_hours"))
    s_state[prefix + "trainer_revision_hours_noise"] = tc.get("revision_hours_noise", s_state.get(prefix + "trainer_revision_hours_noise"))
    s_state[prefix + "trainer_avg_initial_quality"] = tc.get("average_initial_quality", s_state.get(prefix + "trainer_avg_initial_quality"))
    s_state[prefix + "trainer_avg_initial_quality_noise"] = tc.get("average_initial_quality_noise", s_state.get(prefix + "trainer_avg_initial_quality_noise"))
    s_state[prefix + "trainer_revision_improvement"] = tc.get("revision_improvement", s_state.get(prefix + "trainer_revision_improvement"))
    s_state[prefix + "trainer_revision_improvement_noise"] = tc.get("revision_improvement_noise", s_state.get(prefix + "trainer_revision_improvement_noise"))
    s_state[prefix + "trainer_revision_priority"] = tc.get("revision_priority", s_state.get(prefix + "trainer_revision_priority"))

    rc = config_data.get("reviewer_config", {})
    s_state[prefix + "reviewer_max_hours_per_week"] = rc.get("max_hours_per_week", s_state.get(prefix + "reviewer_max_hours_per_week"))
    s_state[prefix + "reviewer_target_hours_per_day"] = rc.get("target_hours_per_day", s_state.get(prefix + "reviewer_target_hours_per_day"))
    s_state[prefix + "reviewer_target_hours_per_day_noise"] = rc.get("target_hours_per_day_noise", s_state.get(prefix + "reviewer_target_hours_per_day_noise"))
    s_state[prefix + "reviewer_review_hours"] = rc.get("review_hours", s_state.get(prefix + "reviewer_review_hours"))
    s_state[prefix + "reviewer_review_hours_noise"] = rc.get("review_hours_noise", s_state.get(prefix + "reviewer_review_hours_noise"))
    s_state[prefix + "reviewer_quality_threshold"] = rc.get("quality_threshold", s_state.get(prefix + "reviewer_quality_threshold"))
    s_state[prefix + "reviewer_quality_threshold_noise"] = rc.get("quality_threshold_noise", s_state.get(prefix + "reviewer_quality_threshold_noise"))
...
def get_config_from_session_state(s_state, prefix="") -> dict:
    ...
    return {
        ...
        "trainer_config": {
            ...
            "target_hours_per_day_noise": s_state.get(prefix + "trainer_target_hours_per_day_noise"),
            ...
            "average_initial_quality_noise": s_state.get(prefix + "trainer_avg_initial_quality_noise"),
            "revision_improvement": s_state.get(prefix + "trainer_revision_improvement"),
            "revision_improvement_noise": s_state.get(prefix + "trainer_revision_improvement_noise"),
        ...
        "reviewer_config": {
            ...
            "target_hours_per_day_noise": s_state.get(prefix + "reviewer_target_hours_per_day_noise"),
            ...
            "quality_threshold_noise": s_state.get(prefix + "reviewer_quality_threshold_noise"),
        }
    }
...
def init_parameter_session_state():
    ...
    defaults = {
        "simulation_mode": "Single Configuration", # New mode selector
        "num_trainers": 60, "num_reviewers": 40, "simulation_days": 21,
        "trainer_max_hours_per_week": 40.0, "trainer_target_hours_per_day": 5.0, "trainer_target_hours_per_day_noise": 1.0,
        "trainer_writing_hours": 5.0, "trainer_writing_hours_noise": 1.0, "trainer_revision_hours": 3.0, "trainer_revision_hours_noise": 1.0,
        "trainer_avg_initial_quality": 0.5, "trainer_avg_initial_quality_noise": 0.2, "trainer_revision_improvement": 0.1, 
        "trainer_revision_improvement_noise": 0.03,
        "trainer_revision_priority": 0.7,
        "reviewer_max_hours_per_week": 40.0, "reviewer_target_hours_per_day": 5.0, "reviewer_target_hours_per_day_noise": 1.0,
        "reviewer_review_hours": 5.0, "reviewer_review_hours_noise": 1.0, "reviewer_quality_threshold": 0.90, "reviewer_quality_threshold_noise": 0.05,
        "current_simulation_name": "Default Scenario Name",
        # Multi-domain state
...
if st.session_state.simulation_mode == "Single Configuration":
    ...
    if available_presets:
        ...
        if selected_preset_pretty_name_single and selected_preset_pretty_name_single != "None":
            if st.sidebar.button("Load Selected Preset into Current Single Config", key="load_preset_button_single"):
                try:
                    ...
                    with open(os.path.join(CONFIG_DIR, file_to_load), 'r') as f:
                        ...
                        if filename_to_pretty_name(file_to_load[:-5]) != st.session_state.current_simulation_name:
                             st.session_state.current_simulation_name = filename_to_pretty_name(file_to_load[:-5])
                        st.sidebar.success(f"Preset '{selected_preset_pretty_name_single}' loaded!")
                        st.rerun()
                except Exception as e:
...
if st.sidebar.button("Save Current Single Config as Preset", key="save_preset_button_single"):
...
uploaded_config_file = st.sidebar.file_uploader("Upload Configuration JSON", type=["json"], key="config_uploader_single")

if uploaded_config_file is not None:
    current_file_id = uploaded_config_file.file_id # Use .file_id for a unique identifier
    if current_file_id != st.session_state.get('last_uploaded_file_id'):
        try:
            ...
            st.session_state.last_uploaded_file_id = current_file_id # Mark this file ID as processed
            st.sidebar.success("Uploaded configuration loaded!")
        except json.JSONDecodeError:
            st.sidebar.error("Invalid JSON file in uploader.")
            st.session_state.last_uploaded_file_id = None # Allow re-processing if error on this file
        except Exception as e:
...
# Determine default filename for config export
config_export_filename = "simulation_config.json"
current_sim_name_for_file = st.session_state.get("current_simulation_name", "").strip()
if current_sim_name_for_file:
    sluggified_name = pretty_name_to_filename(current_sim_name_for_file)
    if sluggified_name:
        config_export_filename = f"{sluggified_name}_config.json"
...
if st.session_state.simulation_mode == "Single Configuration":
    ...
    st.session_state.num_trainers = st.sidebar.slider("Number of Trainers", 1, 100, st.session_state.num_trainers, key="s_num_trainers_single")
    st.session_state.num_reviewers = st.sidebar.slider("Number of Reviewers", 1, 100, st.session_state.num_reviewers, key="s_num_reviewers_single")
    st.session_state.simulation_days = st.sidebar.slider("Simulation Duration (Days)", 7, 90, st.session_state.simulation_days, 7, key="s_simulation_days_single")

    st.sidebar.subheader("Trainer Agent Configuration (Single Config)")
    st.session_state.trainer_max_hours_per_week = st.sidebar.slider("Trainer: Max Hours/Week", 10.0, 80.0, st.session_state.trainer_max_hours_per_week, 0.5, key="s_trainer_max_hours_week_single")
    st.session_state.trainer_target_hours_per_day = st.sidebar.slider("Trainer: Target Hours/Day", 1.0, 12.0, st.session_state.trainer_target_hours_per_day, 0.5, key="s_trainer_target_hours_day_single")
    st.session_state.trainer_target_hours_per_day_noise = st.sidebar.number_input("└ Target Hours/Day Noise (StdDev)", min_value=0.0, value=st.session_state.trainer_target_hours_per_day_noise, step=0.1, format="%.1f", key="s_trainer_target_hours_noise_single")
    st.session_state.trainer_writing_hours = st.sidebar.slider("Trainer: Writing Hours/Task", 1.0, 20.0, st.session_state.trainer_writing_hours, 0.5, key="s_trainer_writing_hours_single")
    st.session_state.trainer_writing_hours_noise = st.sidebar.number_input("└ Writing Hours/Task Noise (StdDev)", min_value=0.0, value=st.session_state.trainer_writing_hours_noise, step=0.1, format="%.1f", key="s_trainer_writing_hours_noise_single")
    st.session_state.trainer_revision_hours = st.sidebar.slider("Trainer: Revision Hours/Task", 0.5, 10.0, st.session_state.trainer_revision_hours, 0.25, key="s_trainer_revision_hours_single")
    st.session_state.trainer_revision_hours_noise = st.sidebar.number_input("└ Revision Hours/Task Noise (StdDev)", min_value=0.0, value=st.session_state.trainer_revision_hours_noise, step=0.1, format="%.1f", key="s_trainer_revision_hours_noise_single")
    st.session_state.trainer_avg_initial_quality = st.sidebar.slider("Trainer: Average Initial Quality", 0.0, 1.0, st.session_state.trainer_avg_initial_quality, 0.01, key="s_trainer_avg_initial_quality_single")
    st.session_state.trainer_avg_initial_quality_noise = st.sidebar.number_input("└ Avg Initial Quality Noise (StdDev)", min_value=0.0, max_value=0.5, value=st.session_state.trainer_avg_initial_quality_noise, step=0.01, format="%.2f", key="s_trainer_avg_initial_quality_noise_single")
    st.session_state.trainer_revision_improvement = st.sidebar.slider("Trainer: Revision Quality Improvement", 0.0, 0.5, st.session_state.trainer_revision_improvement, 0.01, key="s_trainer_revision_improvement_single")
    st.session_state.trainer_revision_improvement_noise = st.sidebar.number_input("└ Revision Improvement Noise (StdDev)", min_value=0.0, max_value=0.25, value=st.session_state.trainer_revision_improvement_noise, step=0.01, format="%.2f", key="s_trainer_revision_improvement_noise_single")
    st.session_state.trainer_revision_priority = st.sidebar.slider("Trainer: Revision Priority (Prob)", 0.0, 1.0, st.session_state.trainer_revision_priority, 0.05, key="s_trainer_revision_priority_single")

    st.sidebar.subheader("Reviewer Agent Configuration (Single Config)")
    st.session_state.reviewer_max_hours_per_week = st.sidebar.slider("Reviewer: Max Hours/Week", 20.0, 80.0, st.session_state.reviewer_max_hours_per_week, 1.0, key="s_reviewer_max_hours_week_single")
    st.session_state.reviewer_target_hours_per_day = st.sidebar.slider("Reviewer: Target Hours/Day", 1.0, 12.0, st.session_state.reviewer_target_hours_per_day, 0.5, key="s_reviewer_target_hours_day_single")
    st.session_state.reviewer_target_hours_per_day_noise = st.sidebar.number_input("└ Target Hours/Day Noise (StdDev)", min_value=0.0, value=st.session_state.reviewer_target_hours_per_day_noise, step=0.1, format="%.1f", key="s_reviewer_target_hours_noise_single")
    st.session_state.reviewer_review_hours = st.sidebar.slider("Reviewer: Review Hours/Task", 0.5, 10.0, st.session_state.reviewer_review_hours, 0.25, key="s_reviewer_review_hours_single")
    st.session_state.reviewer_review_hours_noise = st.sidebar.number_input("└ Review Hours/Task Noise (StdDev)", min_value=0.0, value=st.session_state.reviewer_review_hours_noise, step=0.1, format="%.1f", key="s_reviewer_review_hours_noise_single")
    st.session_state.reviewer_quality_threshold = st.sidebar.slider("Reviewer: Quality Threshold for Sign-off", 0.0, 1.0, st.session_state.reviewer_quality_threshold, 0.01, key="s_reviewer_quality_threshold_single")
    st.session_state.reviewer_quality_threshold_noise = st.sidebar.number_input("└ Quality Threshold Noise (StdDev)", min_value=0.0, max_value=0.5, value=st.session_state.reviewer_quality_threshold_noise, step=0.01, format="%.2f", key="s_reviewer_quality_threshold_noise_single")
...
# --- UI for Multi-Domain Configuration Mode ---
    ...
    st.sidebar.markdown("Define agent groups for different domains. Each domain uses a global preset for detailed agent characteristics, including number of trainers/reviewers.")
    ...
    if not st.session_state.active_domains:
        ...
        for domain_name in st.session_state.active_domains:
            if domain_name not in st.session_state.domain_configs:
                domain_defaults = PREDEFINED_DOMAINS_SETUP[domain_name]
                st.session_state.domain_configs[domain_name] = {
                    # "num_trainers": domain_defaults["num_trainers"], # Will come from preset
                    # "num_reviewers": domain_defaults["num_reviewers"],  # Will come from preset
                    "preset_filename": domain_defaults["initial_preset_filename"], 
                    "rate": domain_defaults["rate"],
                    "loaded_num_trainers": 0, # For display after preset load
                    "loaded_num_reviewers": 0 # For display after preset load
                }

    for domain_name in list(st.session_state.active_domains): 
        with st.sidebar.expander(f"Domain: {domain_name} (Rate: ${st.session_state.domain_configs[domain_name].get('rate', 'N/A')}/hr)", expanded=True):
            # Removed direct num_trainers/reviewers inputs here
            
            current_preset_file = st.session_state.domain_configs[domain_name].get("preset_filename")
            current_preset_pretty = filename_to_pretty_name(current_preset_file[:-5]) if current_preset_file else "None"

            preset_options = ["None"] + list(available_presets.keys())
            selected_preset_for_domain = st.selectbox(
                f"Agent Config Preset for {domain_name}", 
                options=preset_options, 
                index=preset_options.index(current_preset_pretty) if current_preset_pretty in preset_options else 0,
                key=f"preset_select_{domain_name}"
            )
            ...
            if selected_preset_for_domain != current_preset_pretty: 
                ...
                st.session_state.domain_configs[domain_name]["preset_filename"] = new_preset_filename
                # Attempt to load num_trainers/reviewers from the new preset for display purposes
                if new_preset_filename:
                    try:
                        with open(os.path.join(CONFIG_DIR, new_preset_filename), 'r') as f:
                            preset_data = json.load(f)
                        sim_settings = preset_data.get("simulation_settings", {})
                        st.session_state.domain_configs[domain_name]["loaded_num_trainers"] = sim_settings.get("num_trainers", 0)
                        st.session_state.domain_configs[domain_name]["loaded_num_reviewers"] = sim_settings.get("num_reviewers", 0)
                    except Exception as e:
                        st.warning(f"Could not read agent counts from {new_preset_filename}: {e}")
                        st.session_state.domain_configs[domain_name]["loaded_num_trainers"] = 0
                        st.session_state.domain_configs[domain_name]["loaded_num_reviewers"] = 0
                else:
                    st.session_state.domain_configs[domain_name]["loaded_num_trainers"] = 0
                    st.session_state.domain_configs[domain_name]["loaded_num_reviewers"] = 0
                st.rerun() 
            
            # Display the number of trainers/reviewers from the loaded preset
            if st.session_state.domain_configs[domain_name].get("preset_filename"):
                loaded_trainers = st.session_state.domain_configs[domain_name].get("loaded_num_trainers", "N/A")
                loaded_reviewers = st.session_state.domain_configs[domain_name].get("loaded_num_reviewers", "N/A")
                st.markdown(f"_Trainers from preset: **{loaded_trainers}**_" )
                st.markdown(f"_Reviewers from preset: **{loaded_reviewers}**_" )
            else:
                st.markdown("_Select a preset to define agent counts and configs._")
    ...
    st.session_state.simulation_days = st.sidebar.slider(
        ...
        7, 90, 
        st.session_state.get("simulation_days", 21), # Use general simulation_days from session_state
        7, 
...
if st.sidebar.button("Run Simulation", key="run_sim_button"):
    ...
    if st.session_state.simulation_mode == "Single Configuration":
        ...
        domain_name_single = st.session_state.get("current_simulation_name", "Default").strip() or "Default"
        ...
        for domain_name in st.session_state.active_domains:
            domain_detail = st.session_state.domain_configs[domain_name]
            preset_filename_for_domain = domain_detail.get("preset_filename")

            if not preset_filename_for_domain:
                st.sidebar.error(f"Error: Domain '{domain_name}' is missing a selected agent config preset.")
            ...
            try:
                ...
                with open(os.path.join(CONFIG_DIR, preset_filename_for_domain), 'r') as f:
                    preset_data = json.load(f)
                ...
                # Update the session state just in case it wasn't updated by UI interaction (e.g. if app started with preset already selected)
                st.session_state.domain_configs[domain_name]["loaded_num_trainers"] = num_trainers_from_preset
                st.session_state.domain_configs[domain_name]["loaded_num_reviewers"] = num_reviewers_from_preset
                
                if num_trainers_from_preset == 0 and num_reviewers_from_preset == 0:
                    st.sidebar.warning(f"Skipping domain '{domain_name}' as its selected preset ('{preset_filename_for_domain}') has 0 trainers and 0 reviewers.")
                    continue
                ...
                domain_setups_for_sim.append(
                    DomainSimulationSetup(
                        ...
                        num_trainers=num_trainers_from_preset, # Use count from preset
                        num_reviewers=num_reviewers_from_preset, # Use count from preset
                        trainer_cfg=domain_trainer_cfg,
                        reviewer_cfg=domain_reviewer_cfg
                    )
                )
            except FileNotFoundError:
                st.sidebar.error(f"Preset file {preset_filename_for_domain} not found for domain {domain_name}.")
                any_domain_misconfigured = True
            except Exception as e:
                st.sidebar.error(f"Error loading preset for domain {domain_name}: {e}")
                any_domain_misconfigured = True
        
        if any_domain_misconfigured or not domain_setups_for_sim:
            st.sidebar.error("Multi-domain simulation cannot run due to configuration errors or no valid domains.")
            st.session_state.simulation_run = False
        else:
             sim_days = st.session_state.simulation_days # Ensure sim_days is set for multi-domain

    if domain_setups_for_sim: # Proceed only if there are valid setups
        sim_cfg_object_local = SimulationConfig(
            simulation_days=sim_days, # Use the sim_days determined by mode
        ...
        st.session_state.final_sim_config = sim_cfg_object_local # Store the successful config
    else:
        st.sidebar.warning("No valid simulation setups to run. Please check configurations.")
...
if st.session_state.simulation_run and hasattr(st.session_state, 'df_summary'):
    ...
    if st.session_state.simulation_mode == "Multi-Domain Configuration":
        ...
        if final_config is not None and hasattr(final_config, 'domain_setups') and final_config.domain_setups:
             active_domain_names_display = [ds.domain_name for ds in final_config.domain_setups]
        
        if active_domain_names_display:
            st.subheader(f"Results for Multi-Domain Simulation: {', '.join(active_domain_names_display)}")
        else:
    ...
    st.line_chart(df_summary.set_index('day')[[
        'new_tasks_created',
    ...
    status_colors = ["#D3D3D3",  # 1. Claimed (Light Gray)
    ...
    for original_key, display_name in ordered_status_map.items():
        if original_key in df_summary.columns:
            ...
            df_plot_ready[display_name] = 0 # Add column with zeros if missing from source
    ...
    if not df_plot_ready.empty:
        ...
        st.caption("This chart shows the cumulative number of tasks in each status at the end of each day.")
    else:
    ...
    with col2:
        ...
        st.line_chart(df_summary.set_index('day')[
            ['avg_trainer_hrs_worked_today', 'avg_reviewer_hrs_worked_today'] # Removed weekly averages
        ])
    ...
    daily_summary_filename = "daily_simulation_summary.csv"
    if current_sim_name_for_file:
        sluggified_name = pretty_name_to_filename(current_sim_name_for_file)
        if sluggified_name:
            daily_summary_filename = f"{sluggified_name}_daily_summary.csv"
    st.download_button(
    ...
    if hasattr(st.session_state, 'tasks_final_state') and st.session_state.tasks_final_state:
        ...
        final_tasks_cols_order = ["ID", "Status", "Owner ID", "Owner Domain", "Reviewer ID", "Reviewer Domain", "Quality", "Revisions", "Review Cycles", "Major Issues", "Minor Issues"]
        ...
        final_tasks_filename = "final_task_list.csv"
        if current_sim_name_for_file:
            sluggified_name = pretty_name_to_filename(current_sim_name_for_file)
            if sluggified_name:
                final_tasks_filename = f"{sluggified_name}_final_tasks.csv"
        st.download_button(
        ...
        if hasattr(st.session_state, 'sim_trainers') and st.session_state.sim_trainers:
            for trainer in st.session_state.sim_trainers:
                tasks_created = sum(1 for t in st.session_state.tasks_final_state if t.owner_id == trainer.id)
                signed_off_owned_tasks = [t for t in st.session_state.tasks_final_state if t.owner_id == trainer.id and t.status == TaskStatus.SIGNED_OFF]
                tasks_signed_off_owned_count = len(signed_off_owned_tasks)
                avg_major_issues_owned_signed_off = float(np.mean([t.major_issues for t in signed_off_owned_tasks])) if signed_off_owned_tasks else 0.0
                avg_minor_issues_owned_signed_off = float(np.mean([t.minor_issues for t in signed_off_owned_tasks])) if signed_off_owned_tasks else 0.0
                avg_quality_owned_signed_off = float(np.mean([t.quality_score for t in signed_off_owned_tasks])) if signed_off_owned_tasks else 0.0
                avg_review_cycles_owned_signed_off = float(np.mean([t.revision_count + 1 for t in signed_off_owned_tasks])) if signed_off_owned_tasks else 0.0
                tasks_needing_work_owned = sum(1 for t in st.session_state.tasks_final_state if t.owner_id == trainer.id and t.status == TaskStatus.NEEDS_WORK)
        ...
        if hasattr(st.session_state, 'sim_reviewers') and st.session_state.sim_reviewers:
            for reviewer in st.session_state.sim_reviewers:
                signed_off_reviewed_tasks = [t for t in st.session_state.tasks_final_state if t.reviewer_id == reviewer.id and t.status == TaskStatus.SIGNED_OFF]
                tasks_signed_off_by_reviewer_count = len(signed_off_reviewed_tasks)
                avg_major_issues_reviewed_signed_off = float(np.mean([t.major_issues for t in signed_off_reviewed_tasks])) if signed_off_reviewed_tasks else 0.0
                avg_minor_issues_reviewed_signed_off = float(np.mean([t.minor_issues for t in signed_off_reviewed_tasks])) if signed_off_reviewed_tasks else 0.0
                avg_quality_reviewed_signed_off = float(np.mean([t.quality_score for t in signed_off_reviewed_tasks])) if signed_off_reviewed_tasks else 0.0
                avg_review_cycles_reviewed_signed_off = float(np.mean([t.revision_count + 1 for t in signed_off_reviewed_tasks])) if signed_off_reviewed_tasks else 0.0
                tasks_reviewed_by_agent = sum(1 for t in st.session_state.tasks_final_state if t.reviewer_id == reviewer.id)
        ...
        if agent_performance_data:
            ...
            column_order = [
                "Agent ID", "Type", "Domain",
                "Tasks Created", "Tasks Signed Off (Owned)", 
                "Avg Quality (Signed Off Owned)", "Avg Review Cycles (Signed Off Owned)",
                "Avg Major Issues (Signed Off Owned)", "Avg Minor Issues (Signed Off Owned)", 
                "Tasks Needing Work (Owned)", "Tasks Reviewed", "Tasks Signed Off (Reviewed)",
                "Avg Quality (Signed Off Reviewed)", "Avg Review Cycles (Signed Off Reviewed)",
                "Avg Major Issues (Signed Off Reviewed)", "Avg Minor Issues (Signed Off Reviewed)"
            ]
            ...
            agent_perf_filename = "agent_performance_summary.csv"
            if current_sim_name_for_file:
                sluggified_name = pretty_name_to_filename(current_sim_name_for_file)
                if sluggified_name:
                    agent_perf_filename = f"{sluggified_name}_agent_performance.csv"
            st.download_button(
        ...
        st.markdown("Note: Plots can be saved directly using the options menu (⋮) on each chart.")        
        col1, col2 = st.columns(2)

        # Trainer Productivity Histogram
        if hasattr(st.session_state, 'sim_trainers') and st.session_state.sim_trainers:
            trainer_signed_off_counts = [
                sum(1 for t in st.session_state.tasks_final_state if t.owner_id == trainer.id and t.status == TaskStatus.SIGNED_OFF)
                for trainer in st.session_state.sim_trainers
            ]
            ...
            with col1:
                ...
                if trainer_signed_off_counts:
                    source = pd.DataFrame({'Tasks Signed Off (Owned)': trainer_signed_off_counts})
                    chart = alt.Chart(source).mark_bar().encode(
                        alt.X('Tasks Signed Off (Owned):Q', bin=alt.Bin(maxbins=10), title='Tasks Signed Off (Owned)'),
                        alt.Y('count()', title='Number of Trainers')
                    ).properties(
                        title='Trainer Output Distribution'
                    )
        ...
        if hasattr(st.session_state, 'sim_reviewers') and st.session_state.sim_reviewers:
            reviewer_signed_off_counts = [
                sum(1 for t in st.session_state.tasks_final_state if t.reviewer_id == reviewer.id and t.status == TaskStatus.SIGNED_OFF)
                for reviewer in st.session_state.sim_reviewers
            ]

            with col2:
                st.markdown("**Reviewer Productivity** (Tasks Signed Off - Reviewed by them)")
                if reviewer_signed_off_counts:
                    source_rev = pd.DataFrame({'Tasks Signed Off (Reviewed)': reviewer_signed_off_counts})
                    chart_rev = alt.Chart(source_rev).mark_bar().encode(
                        alt.X('Tasks Signed Off (Reviewed):Q', bin=alt.Bin(maxbins=10), title='Tasks Signed Off (Reviewed)'),
                        alt.Y('count()', title='Number of Reviewers')
                    ).properties(
                        title='Reviewer Output Distribution'
                    )
    ...
    elif hasattr(st.session_state, 'tasks_final_state') and not st.session_state.tasks_final_state:
